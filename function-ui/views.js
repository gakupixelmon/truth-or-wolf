...
export function createViews({ state }) {
  ...
  function rightPanel() {
    ...
    const investigationRule = state.game.rules?.limitInvestigatorsPerTarget ? `同一対象の占い上限は${state.game.rules.maxInvestigatorsPerTarget}人（人間は先着、CPU同士は無作為）。` : "同一対象への占い人数制限はありません。";
